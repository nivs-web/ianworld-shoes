...
function meRecord(profile, fb) {
  return {
    ...
    /**
     * ★ **드래곤 번호도 싣는다.** (2026-08-26)
     * 결투 결과 화면이 `p.dragon` 으로 상대 얼굴을 그리는데 아무도 안 써 줘서
     * **늘 0번(노바트)** 으로 보였다. 신발방에서는 안 쓰이지만 숫자 하나라 부담이 없고,
     * 게임마다 다른 기록을 만들면 대기방 코드가 두 갈래로 갈린다.
     */
    dragon: profile.dragonCharacter | 0,
    /**
     * ★ **지갑의 금화.** (2026-08-27, 사용자 지정)
     * *"게임 방입장 하면 상대 캐릭터 부분에 남아있는 금화가 떠야지"*
     *
     * 결투는 금화를 걸고 하는 것이라 **상대가 얼마를 들고 있는지**가
     * 이 방에서 가장 궁금한 숫자다. 이름은 `coins` 가 아니라 `wallet` 이다 —
     * `coins` 는 이미 **이번 판에서 주운 금화**로 쓰고 있어서 겹치면
     * 결투 정산이 지갑을 상금으로 오해한다.
     */
    wallet: profile.dragonCoins ?? 0,
    /**
