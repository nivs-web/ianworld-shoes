 ...
 *
 * ★ **실측한 밸런스** (1스테이지, 보통, 봇 4회 반복)
 *   맨몸   79초 (4회 모두) / 평균 69금화
 *   풀장착 74~75초          / 평균 126금화
 *
 * 시간은 5%밖에 안 줄고 **금화만 거의 두 배로 늘어난다.** 의도한 대로다 —
 * 무장이 하는 일은 판을 빨리 끝내는 것이 아니라 한 판에서 더 많이 주워오는 것이다.
 * 사용자가 정한 한 판 19분은 그대로 지켜진다.
 *
 * 공격력만 따로 재 보았을 때도 평평했다 (1.00→1.24 에서 79초→74초).
 * 숫자를 고치거든 **반드시 다시 재라** — 한 번만 재면 편차가 커서
 * 처음엔 79초 → 40초 로 절반이 된 것처럼 보였고, 그건 잔음이었다.
 */
