...
import { collection as getCollection, dexUnique } from '../services/profile.js';
...
function detailPopup(shoe, record) {
  ...
  const overlay = el('div.dialog-overlay', { onclick: close }, [
    el('div.shoe-detail', { onclick: (e) => e.stopPropagation() }, [
      ...
      el('div.shoe-meta', record
        ? `${new Date(record.firstFoundAt).toLocaleDateString('ko-KR')} · ${record.count}회`
        : S.notFoundYet),
  ...
  return {
    render() {
      ...
      return screen(
        ...
        el('div.dex-total', `${S.totalShoes} ${dexUnique()}/${SHOE_TOTAL}`),
        tabs,
