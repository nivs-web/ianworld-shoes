...
export const MULTI = {
  ...
  absentSeconds: 30,
  /**
   * ★ **본인이 "나갔다"고 말한 경우의 유예 — 6초.** (2026-08-21 31차, 사용자 지정)
   *
   * *"튕기거나 홈버튼 눌러서 나간 사람 판단해서 게임 종료되게 만들자"*
   *
   * 위의 30초와 **재는 대상이 다르다.**
   *
   *   · `absentSeconds` 30초 — **조용함**. 신호가 안 오는 것뿐이라 확신이 없다.
   *     탭이 뒤로 갔을 수도, 회선이 잠깐 흔들렸을 수도 있다. 모르는 것을 근거로
   *     남을 판에서 빼면 안 되므로 넉넉하게 기다린다(§9-0-39 의 그 교훈).
   *   · `awaySeconds` 6초 — **명시적 신호**. `awayAt`(홈 버튼을 누른 본인이 찍는다)
   *     이나 `offAt`(소켓이 끊긴 것을 서버가 찍는다)이 있으면 짐작할 게 없다.
   *
   * 6초인 이유는 **나가기 버튼의 유예와 같은 값**이기 때문이다. 사용자에게는 홈
   * 버튼도 나가기다 — 두 길이 같은 시간을 주지 않으면 "홈 버튼으로 나가면 더 오래
   * 버틴다"가 되어 그쪽이 요령이 된다.
   *
   * 짧아도 안전한 이유: 도장이 아니라 **살아 있는 값**이라 돌아와서 지우면 그 즉시
   * 판정이 원상 복구된다. 셋 이상인 판은 전원이 빠져야 끝나므로 잠깐 빠졌다고
   * 판이 끝나지도 않는다.
   */
  awaySeconds: 6,
  /**
   * ★ **아웃체크중 — 자리를 비우면 30초를 세고, 그 안에 안 오면 진다.** (2026-08-21 33차, 사용자 지정)
   *
   * *"게임하다가 중간에 홈버튼 전화가 와서 전화 받거나 해도, '아웃체크중' 이라고 뜨면서
   *   30초 뜨게 만들자, 30초 안에 방에 복귀 안하면 그냥 승리하는걸로 (…)
   *   단 이것도 악용할 수 있으니 딱 1회만 가능하도록"*
   *
   * 31차에는 홈 버튼도 **6초**(`awaySeconds`)로 잘랐다. 판정은 정확했지만 **사람에게는
   * 사고처럼 보였다** — 전화 한 통에 판이 날아가는데 화면에는 아무 설명도 없었다.
   * 그래서 시간을 주되 **무슨 일이 일어나는지 보여 준다**: 남은 사람의 화면에
   * `아웃체크중` 과 빨간 숫자가 뜨고, 0이 되면 그 사람이 이긴다.
   *
   * ## 왜 딱 1회인가
   *
   * 30초는 남은 사람이 **그냥 서서 기다리는 시간**이다. 무제한이면 29초마다 들락거리며
   * 상대를 영원히 붙잡을 수 있다 — 26차에 일시정지를 1인 1회로 묶은 것과 **같은 악용**이다.
   * 두 번째부터는 `awaySeconds`(6초)로 돌아간다. 즉 **긴 유예가 1회**이고 짧은 유예는 늘 있다.
   *
   * 횟수는 `players/$uid/awayCount` 로 서버에 남는다 — **돌아올 때 올린다.** 안 돌아온
   * 사람에게는 횟수가 의미가 없고(어차피 진다), 돌아온 사람만 두 번째부터 짧아지면 된다.
   */
  outCheckSeconds: 30,
  /** 긴 유예(아웃체크)를 몇 번까지 주나 — 사용자 지정 "딱 1회만" */
  outCheckAllowance: 1,
  /**
   * ★ **접속 목록에서 살아 있다고 보는 시간 — 60초.** (2026-08-19 19차 사용자 지정)
   *
   * 여기 있는 이유: 33차부터 **방 목록이 이 값을 함께 본다.** 방에 앉아 있는 사람은
   * 입력이 없어도 15초마다 심장 박동을 보내므로(`presence.heartbeat` 의 `inGame` 예외),
   * **방에는 있는데 접속 목록에 없으면 확실히 나간 것**이다. 두 화면이 서로 다른 숫자를
   * 쓰면 "현재접속자는 0명인데 방은 3개"가 그대로 다시 생긴다.
   */
  onlineSeconds: 60,
  heartbeatMs: 5000,
  /**
