...
export const S = {
  ...
  /**
   * ★ 로비 보유신발 — **숫자를 비트맵으로 크게** 찍으므로 라벨과 단위를 따로 둔다.
   * (2026-08-19 16차, 사용자 지정: *"'보유신발 00켤레' 으로 변경하고 00숫자 크기를
   * 위의 최고기록 계단 표기된 숫자와 같은 크기로"*)
   * 대기방 줄은 한 줄에 한 명씩이라 그대로 문장형이다(`playerShoesOwned`).
   */
  myShoesLabel: '보유신발',
  myShoesUnit: '켤레',
  /**
   * ★ 띄어쓰기 (2026-08-19 17차, 사용자 지정)
   * *"신발도감 87/130켤레, 멀티게임 17승/50게임 이렇게 띄어쓰기 없이 붙여"*
   * 이름도 붙이고 빗금 앞뒤도 붙인다 — 숫자만 크게 찍으므로(`.stat-num`)
   * 빈칸이 있으면 큰 숫자와 작은 글자 사이가 더 벌어져 보인다.
   * (같은 회차 안에서 한 번 띄웠다가 되돌린 것이다 — 사용자 지정이 바뀌었다)
   */
  myDexProgress: (have, all) => `신발도감 ${have}/${all}켤레`,
