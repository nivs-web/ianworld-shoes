...
export const S = {
  ...
  rankUnitWins: '승',
  /**
   * 승률왕 맨 위 안내. **목록보다 먼저** 뜬다 — 10판을 안 채운 사람이 자기 이름을
   * 찾다가 "안 나온다"고 여기지 않게, 규칙을 먼저 말한다.
   */
  rateKingNotice: (n) => `승률왕은 최소 멀티게임을 ${n}판 이상 유저만 측정합니다`,
  /** 승률 줄 — 다른 탭의 `0켤레` 가 있던 그 자리에 들어간다 (사용자 지정 문구 그대로) */
  rateLine: (games, wins, pct) => `총 ${games}게임중 ${wins}승으로 ${pct}% 승률`,
  rateNone: '기록 없음',
  /**
  ...
  /**
   * 신발왕 탭의 승패 칸. **`멀티승률` 이라는 머리말을 뗐다** (2026-08-19 23차, 사용자 지정)
   * — 이름 바로 옆에 붙는 칸이라 머리말이 자리만 먹고, 무엇의 숫자인지는 `승`·`멀티게임`
   * 두 단어가 이미 말한다.
   */
  rankWinRate: (wins, games) => `${wins}승/총${games}멀티게임`,
  userCardTitle: '유저상태창',
