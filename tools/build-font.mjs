...
/**
 * ★ **숫자 `4` 만 우리가 그린 것으로 갈아 끼운다.** (2026-08-21 31차, 사용자 신고)
 *
 * *"숫자 4가 깨져서 안보여, 숫자만 다른 폰트로 숫자 4라는게 안깨지게 고쳐"*
 *
 * 갈무리11 **볼드**의 `4` 는 대각선과 세로 기둥이 2도트씩인데 그 사이 빈칸이 위쪽
 * 세 줄에서 통째로 메워진다 — 구운 결과가 이렇다:
 *
 *     ...###..      ..####..      ..####..      ← 여기까지 구멍이 없다
 *     .##.##..      .##.##..      ##..##..
 *     ##..##..      #######.      #######.
 *
 * 그래서 배율 1·2·3 어디서 봐도 **꼭대기가 통째로 뭉친 덩어리**로 보인다. 다른 아홉
 * 자는 멀쩡하다(0·6·8·9 는 구멍이 3도트다) — 열 자를 다 갈아 치울 이유가 없고, 폭이
 * 바뀌면 이미 자로 재 둔 문구 폭 검사가 통째로 흔들린다.
 *
 * 대신 갈무리9 가 쓰는 **각진 4**(대각선 없이 왼팔·가로대·기둥)를 볼드 굵기로 옮겨
 * 그렸다. 획 2도트 · 구멍 3도트로 `0` 과 같은 규격이고, 폭도 8 그대로다.
 *
 * 표를 여기 둔 이유: 굽는 자리와 고치는 자리가 갈리면 다음에 폰트를 다시 구울 때
 * 조용히 원래 글리프로 돌아간다(§9-0-35 의 간판이 정확히 그랬다).
 */
const GLYPH_FIX = {
  '4': [
    '##...##.',
...
for (const ch of CHARS) {
  const fix = GLYPH_FIX[ch];
  if (fix) {
    const w = fix[0].length;
    if (fix.length !== H) throw new Error(`GLYPH_FIX['${ch}'] 는 ${H}줄이어야 한다 (${fix.length}줄)`);
    if (fix.some((r) => r.length !== w)) throw new Error(`GLYPH_FIX['${ch}'] 줄 길이가 다르다`);
    glyphs[ch] = { w, r: fix.map((row) => [...row].reduce((b, c) => (b << 1) | (c === '.' ? 0 : 1), 0)) };
    if (w > maxW) maxW = w;
    continue;
  }
