...
### 2026-08-27 — `rooms/$code/players/$uid/wallet` 추가

결투는 금화를 걸고 하는 것이라 대기방에서 **상대가 얼마를 들고 있는지**를 띄운다.
이름을 `coins` 로 하면 안 된다 — 그건 이미 **이번 판에서 주운 금화**로 쓰고 있어서
겹치면 결투 정산이 지갑을 상금으로 오해한다.

```
"wallet": { ".validate":
  "newData.isNumber() && newData.val() >= 0 && newData.val() <= 100000000
   && ($uid == auth.uid || newData.val() == data.val())" }
```

콘솔에 게시 완료. 게시 후 배포본을 다시 읽어 이 저장소 파일과 대조했다 —
규칙 경로 110개가 이름·구조까지 전부 같다.

### 2026-08-27 — `presence/$uid/coins` 추가

현재접속자 목록이 드래곤 쪽에서도 "신발 0켤레" 를 띄우고 있었다. 이 게임의
화폐는 금화이므로 접속 카드에 지갑을 같이 싣는다.
...
콘솔에 게시 완료. 게시 후 다시 읽어 확인했다 (presence 필드 13개, wallet 도 그대로).
